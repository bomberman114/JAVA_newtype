...
public class Javaswing {
	...
	boolean count;
			...
			// new --> 객체의 인스턴스화 
			// 전역변수 자리에 JFram frame;  --> 이건 그냥. frame은 JFrame이다. 라고 선언만 한 것. 
	...
	public void Layout() {
		...
		// button 생성, frame과 panel안에 뭐가 들어가있을까? -> 아무것도 안들어가있음.
		...
		// button 생성 끝, JFrame과 JPanel안에 뭐가 들어가있을까? -> 아무것도 안들어가있다.
		
		result = new JLabel("0", JLabel.RIGHT);

		panel.setLayout(null);

		// panel에 요소들 추가 JFrame과 JPanel안에 뭐가 들어가있을까? -> 
		panel.add(result);
		panel.add(btn_plus);
		panel.add(btn_minus);
		panel.add(btn_mul);
		...
		// JFrame과 JPanel안에 뭐가 들어가있을까? -> panel.add(버튼들) -> 이제 panel이라는 변수안에 위에서 만든 버튼이 들어감. 
		// 버튼을 그냥 뭐 넣은거지 정렬하지 않음. 
		...
		// 이벤트 발생 addActionListener -> 무언가가 클릭되었을 때(대표적으로는 버튼), addMouseListener ? 이런 것도 있다.
		// 이게 마우스가 클릭되었을 때, 클릭하는 중일 때, 클릭하다가 손을 땟을 때, 마우스포인터가 어딘가에 들어갔을 때, 어딘가에서 나왔을 때 이렇게 지정가능함.
		
		// 이벤트를 연결해주는 곳. -> 이거 안해주면 버튼 클릭했을 때 아무런 반응도 없음. 생명을 불어넣는다라는 느낌. 이거 안해주면 그냥 버튼이라는 모양만 있는 거
		...
		// 이제 여기서 frame에다가 panel을 집어넣는다. -> panel안에는 뭐가 있을까? : 여러가지 버튼인데!!!! addActionListener(이벤트 연결)이 되어 있는 버튼들을!! 가지고 있다.
		frame.add(panel);
		
		
		// 여기서 이제 다 집어넣고!!!!! 종료버튼, 사이즈, 무조건 제일 마지막으로 setVisible (눈에 보이도록 하겠다) 선언
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frame.setSize(400, 500);
		frame.setTitle("이거슨 계산기다!!!!!");
		frame.setVisible(true);
		
		//여기까지가 버튼만들고 버튼에 이벤트 연결하고 panel안에 넣고 frame넣고 실행하는 것까지 모조리 
	}
